...
export default async function EditDocumentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [doc, customers, jobs, issuers, quotations] = await Promise.all([
    getDocument(id), getCustomers(), getJobs(), getIssuers(), getQuotations(),
  ])
  if (!doc) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <DocumentForm document={doc} customers={customers} jobs={jobs} issuers={issuers} quotations={quotations} />
    </div>
  )
}
