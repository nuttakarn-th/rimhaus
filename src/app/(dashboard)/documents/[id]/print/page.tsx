...
export default async function DocumentPrintPage({
  ...
  const title = titleMap[doc.doc_type] ?? doc.doc_type
          ...
          <div className="p-8 print:p-0">
              ...
              <div className="text-sm leading-relaxed">
                {doc.issuer_header_image_url && (
                  <div className="mb-2">
                    ...
                    <img src={doc.issuer_header_image_url} alt="Logo" className="max-h-14 max-w-[260px] object-contain" />
                  </div>
                )}
                <div className="font-bold text-base text-[hsl(25,20%,10%)]">{doc.issuer_name ?? "—"}</div>
                {doc.issuer_address && (
                  <div className="text-[hsl(25,10%,35%)] whitespace-pre-line mt-0.5 text-xs leading-snug">{doc.issuer_address}</div>
                )}
                ...
                {doc.issuer_id_card && (
                  <div className="text-[hsl(25,10%,35%)] text-xs mt-0.5">เลขที่บัตรประชาชน {doc.issuer_id_card}</div>
                )}
              </div>
            ...
            {doc.platforms && doc.platforms.length > 0 && (
              <div className="mb-4 flex gap-4 text-sm">
                ...
                <span className="text-[hsl(25,10%,35%)]">
                  {doc.platforms.map(p => {
              ...
              <tbody>
                ...
                {Array.from({ length: Math.max(0, 3 - (doc.document_items?.length ?? 0)) }).map((_, i) => (
                  <tr key={`empty-${i}`} className="border-b border-[hsl(35,20%,88%)]">
                    ...
                    <td className="py-1.5 px-3" /><td className="py-1.5 px-3" /><td className="py-1.5 px-3" /><td className="py-1.5 px-3" />
                  </tr>
                ))}
              </tbody>
              <tfoot>
                  ...
                  {(isInvoice || isReceipt) ? (
                      ...
                      <table className="w-full text-xs border-collapse">
                        <tbody>
                          <tr>
                            ...
                            <td className="py-1 px-3 text-right border-b border-[hsl(35,20%,88%)]">{doc.subtotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</td>
                          </tr>
                          {hasDiscount && (
                            <tr>
                              <td className="py-1 px-3 text-[hsl(25,10%,45%)] border-b border-[hsl(35,20%,88%)]">ส่วนลด {doc.discount_type === "%" ? `${doc.discount_value}%` : ""}</td>
                          ...
                          {(hasWht || isGrossup) && (
                            <tr>
                              ...
                              <td className="py-1 px-3 text-right border-b border-[hsl(35,20%,88%)]">{doc.wht_amount.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</td>
                            </tr>
                          )}
                          <tr className="bg-[hsl(195,60%,88%)]">
                            <td className="py-1.5 px-3 font-bold text-[hsl(25,20%,10%)]">จำนวนเงินทั้งสิ้น</td>
                            <td className="py-1.5 px-3 text-right font-bold text-[hsl(25,20%,10%)]">{displayTotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</td>
                          </tr>
                        </tbody>
                      </table>
                    </td>
                    ...
                    <>
                      ...
                      <td className="py-2 px-3 text-right">
                        {hasDiscount && (
                          <>
                            <div className="text-[hsl(25,10%,40%)] text-xs">{doc.subtotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</div>
                ...
                {remarkLines.length > 0 && (
                  <div>
                    ...
                    <ul className="mt-0.5 space-y-0.5">
                      {remarkLines.map((line, i) => <li key={i} className="flex gap-1.5"><span>●</span><span>{line}</span></li>)}
                    </ul>
                  </div>
                )}
                {doc.payment_terms && (
                  <div>
                    ...
                    {doc.payment_terms.split("\n").filter(Boolean).map((line, i) => <div key={i} className="mt-0.5">● {line}</div>)}
                  </div>
                )}
                {hasBank && (
                  <div>
              ...
              <div className="border border-[hsl(35,20%,80%)] rounded p-3 text-xs text-center space-y-1.5">
                ...
                <div className="flex items-end justify-center h-10">
                  {doc.issuer_signature_url
                    ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={doc.issuer_signature_url} alt="ลายเซ็น" className="max-h-9 max-w-[140px] object-contain" />
                    : <div className="h-8" />
                  }
                </div>
                <div className="border-b border-[hsl(25,20%,40%)] mx-4" />
            ...
            {isQuotation && (doc.issuer_contact_line || doc.issuer_email) && (
              <div className="mt-4 text-xs text-[hsl(25,10%,40%)]">
                {doc.issuer_contact_line
                  ? <>ยืนยันใบเสนอราคา : โอนเงินและส่ง Line ยืนยันที่{" "}<span className="font-semibold text-[hsl(25,20%,15%)]">{doc.issuer_contact_line}</span></>
                  : <>ยืนยันใบเสนอราคา : โอนเงินและส่งเมลยืนยันที่{" "}<a href={`mailto:${doc.issuer_email}`} className="underline text-[hsl(24,85%,50%)]">{doc.issuer_email}</a></>
                }
              </div>
            )}
