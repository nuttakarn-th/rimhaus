...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = settings?.og_title ?? `Rate Card · ${settings?.page_name ?? "Rimhaus"}`
  const description = settings?.og_description ?? `Rate Card, ราคา, และผลงานของ ${settings?.page_name ?? "Content Creator"}`
    ...
    <>
      ...
      <footer className="bg-[hsl(22,18%,10%)] border-t border-white/5">
        <div className="max-w-6xl mx-auto px-6 sm:px-10 py-6 sm:py-8 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8">
          ...
          <Link
            href="/"
            className="font-black text-white text-base sm:text-xl shrink-0 hover:text-white/80 transition-colors"
            style={{ fontFamily: "var(--font-inter, 'Inter', system-ui, sans-serif)" }}
          >
          ...
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex flex-wrap gap-x-5 gap-y-1.5">
              <Link href="/" className="text-sm text-white/50 hover:text-white/90 transition-colors">หน้าหลัก</Link>
