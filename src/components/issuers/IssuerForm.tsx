...
import { upsertIssuer, updateIssuerSignature } from "@/actions/issuers.actions"
...
export function IssuerForm({ issuer }: { issuer?: IssuerProfile }) {
  ...
  async function handleSignatureUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setForm(p => ({ ...p, signature_url: publicUrl }))
    if (issuer?.id) {
      await updateIssuerSignature(issuer.id, publicUrl)
      toast.success("อัปโหลดลายเซ็นสำเร็จ")
    } else {
      toast.success("อัปโหลดลายเซ็นสำเร็จ — กด 'บันทึก' เพื่อยืนยัน")
    }
    setUploading(false)
  }
  ...
  return (
    <div className="space-y-5 max-w-xl">
      ...
      <div className="bg-white rounded-xl border border-[hsl(35,20%,88%)] p-5 space-y-3">
        ...
        <Button size="sm" variant="outline" onClick={() => sigRef.current?.click()} disabled={uploading}>
          <Upload className="w-3.5 h-3.5 mr-1.5" />
          {uploading ? "กำลังอัปโหลด..." : form.signature_url ? "เปลี่ยนลายเซ็น" : "อัปโหลดลายเซ็น PNG"}
        </Button>
