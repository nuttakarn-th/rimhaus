"use client"

import { useState } from "react"
import { useRouter } from "next/navigation"
import { disconnectSocial, refreshInstagramInsights } from "@/actions/social.actions"
import { Button } from "@/components/ui/button"
import { toast } from "sonner"
import { Link2Off, RefreshCw } from "lucide-react"

interface Props {
  isConnected: boolean
  igUsername: string | null
}

export function InstagramSettingsClient({ isConnected, igUsername }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  async function handleDisconnect() {
    if (!confirm(`ยกเลิกการเชื่อม Instagram (@${igUsername}) ใช่มั้ย?`)) return
    setLoading(true)
    const result = await disconnectSocial("instagram")
    setLoading(false)
    if (result.success) {
      toast.success("ยกเลิกการเชื่อม Instagram แล้ว")
      router.refresh()
    } else {
      toast.error(result.error)
    }
  }

  async function handleRefresh() {
    setLoading(true)
    const result = await refreshInstagramInsights()
    setLoading(false)
    if (result.success) {
      toast.success("อัปเดต Instagram Insights สำเร็จ")
      router.refresh()
    } else {
      toast.error(result.error)
    }
  }

  if (isConnected) {
    return (
      <div className="bg-white rounded-xl border border-[hsl(35,20%,88%)] p-5 space-y-3">
        <h2 className="text-sm font-semibold text-[hsl(25,20%,15%)]">จัดการการเชื่อมต่อ</h2>
        <div className="flex gap-2 flex-wrap">
          <Button
            size="sm"
            variant="outline"
            onClick={handleRefresh}
            disabled={loading}
          >
            <RefreshCw className={`w-3.5 h-3.5 mr-1.5 ${loading ? "animate-spin" : ""}`} />
            อัปเดต Insights
          </Button>
          <Button
            size="sm"
            variant="outline"
            className="text-red-500 border-red-200 hover:bg-red-50 hover:text-red-600"
            onClick={handleDisconnect}
            disabled={loading}
          >
            <Link2Off className="w-3.5 h-3.5 mr-1.5" />
            ยกเลิกการเชื่อม
          </Button>
        </div>
      </div>
    )
  }

  return (
    <div className="bg-white rounded-xl border border-[hsl(35,20%,88%)] p-5 space-y-4">
      <h2 className="text-sm font-semibold text-[hsl(25,20%,15%)]">เชื่อม Instagram</h2>
      <a href="/api/instagram/connect">
        <Button className="bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 hover:opacity-90 text-white border-0">
          <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
          เชื่อมต่อกับ Instagram
        </Button>
      </a>
    </div>
  )
}
